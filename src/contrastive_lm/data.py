"""Load and validate contrastive-LM training data."""
...
def read_jsonl(path: str | Path) -> list[dict[str, Any]]:
    path = Path(path)
    if not path.exists():
        raise PipelineError(f"Contrastive-LM data file not found: {path}")
    with path.open(encoding="utf-8") as handle:
        records = [json.loads(line) for line in handle if line.strip()]
    if not records:
        raise PipelineError(f"Contrastive-LM data file is empty: {path}")
    return records


def split_path(data_root: str | Path, dataset_key: str, split: str) -> Path:
    if split not in _SPLITS:
        raise PipelineError(f"Unsupported split {split!r}; expected one of {_SPLITS}.")
...
def load_labeled_texts(
    ...
    """Load target texts for one HT/MT label."""
    records = read_jsonl(split_path(data_root, dataset_key, split))
    required = {"text", "label"}
    missing = required - set(records[0])
    if missing:
        raise PipelineError(f"{dataset_key}/{split} lacks required fields: {sorted(missing)}")
    texts = [
    ...
    if not texts:
        raise PipelineError(f"No non-empty label={label} texts in {dataset_key}/{split}.")
    return texts


def validate_classifier_style_splits(data_root: str | Path, dataset_key: str) -> dict[str, int]:
    """Check class balance and source-group disjointness."""
    ...
    for split in _SPLITS:
        ...
        if any("text" not in record for record in records):
            raise PipelineError(f"{dataset_key}/{split} contains a record without text.")
        source_groups[split] = {
            str(record["source_id"]) for record in records if record.get("source_id") is not None
        }
