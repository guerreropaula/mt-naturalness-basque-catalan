"""Generate and score P4 translations on an evaluation set."""
...
def _build_boundary_prompt_spec(expected_count: int) -> dict[str, str]:
    """Build instructions for a fixed number of sentence units."""
    output_shape = "\n".join(
        f"<s{index}>translation {index}</s{index}>" for index in range(1, expected_count + 1)
    )
    return {
...
def _tag_fixed_sentence_units(source: str, expected_count: int) -> str:
    ...
    if expected_count < 1:
        raise PipelineError("sentence_boundary_count must be positive.")
    ...
    return "\n".join(f"<s{index}>{unit}</s{index}>" for index, unit in enumerate(units, start=1))
...
def _strip_boundary_tags(output: str) -> str:
    ...
    marker = re.compile(rf"</?\s*(?:{aliases})_?\d+\s*>?", flags=re.IGNORECASE)
...
def _generate(
    ...
    *,
    ...
    return [
        text.strip() for text in tokenizer.batch_decode(continuation_ids, skip_special_tokens=True)
    ]
...
def evaluate_sft_split(
    ...
    """Evaluate an adapter on the in-domain test set or a supplied JSONL file."""
    ...
    eval_path = (
        Path(evaluation_file) if evaluation_file is not None else root / "eval" / f"{split}.jsonl"
    )
    records = load_jsonl_records(eval_path)
    if limit is not None:
        records = records[:limit]
    if not records:
        raise PipelineError(f"{split} is empty.")
    ...
    if not adapter_dir.exists():
        raise PipelineError(f"SFT adapter not found: {adapter_dir}")

    result_dataset_key = output_dataset_key or dataset_key
    output_dir = (
        Path(results_root or settings["results_root"]) / result_dataset_key / model_key / split
    )
    ...
    if len(records_by_id) != len(records):
        raise PipelineError(f"{eval_path} contains duplicate evaluation IDs.")
    ...
    if resume and predictions_path.is_file():
        for prediction in load_jsonl_records(predictions_path):
            ...
            if prediction_id in predictions_by_id:
                raise PipelineError(f"Existing predictions contain duplicate ID {prediction_id!r}.")
            ...
            if sentence_boundary_count is not None:
                ...
                reparsed_prediction, boundary_valid, boundary_error = _extract_fixed_sentence_units(
    ...
    if pending_records and model_entry.backend == "vllm":
        ...
        if tokenizer is None:
            raise PipelineError("vLLM did not expose a tokenizer for prompt rendering.")
        ...
        try:
            ...
            raise PipelineError("peft is required for adapter evaluation.") from exc
        transformer_model = PeftModel.from_pretrained(transformer_model, adapter_dir)
        transformer_model.eval()
        transformer_model.generation_config.do_sample = bool(settings["evaluation"]["do_sample"])
        transformer_model.generation_config.num_beams = int(settings["evaluation"]["num_beams"])
        transformer_model.generation_config.temperature = settings["evaluation"].get("temperature")
        ...
        for start_index in range(0, len(pending_records), batch_size):
            ...
            prompts = [
                build_prompt_text(
                    ...
                    target_lang,
                    tokenizer=tokenizer,
                    enable_thinking=bool(settings["chat_format"].get("enable_thinking", False)),
            ...
            if loaded_model is not None:
                ...
                raise PipelineError("No inference backend was loaded.")
    ...
    save_json(
        {
            ...
            "sentence_boundary_contract": {
                ...
                "valid_outputs": (
                    sum(bool(item.get("sentence_boundary_contract_valid")) for item in predictions)
                    if sentence_boundary_count is not None
                    else None
                ),
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate an SFT adapter on the in-domain test set."
    )
    ...
    parser.add_argument(
        "--split", default="test", help="Canonical split label, or a label for --evaluation-file."
    )
    parser.add_argument("--limit", type=int, default=None)
    parser.add_argument("--experiment-key", default="p4_sft")
    parser.add_argument(
        "--evaluation-file", default=None, help="Immutable JSONL with id/source/target fields."
    )
    parser.add_argument(
        "--output-dataset-key", default=None, help="Dataset label used only for result paths."
    )
    parser.add_argument(
        "--max-input-length",
        type=int,
