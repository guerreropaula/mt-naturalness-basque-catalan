...
def build_grpo_records(
    ...
    """Create TRL records while retaining source and reference identifiers."""
    prepared: list[dict[str, str]] = []
    for index, record in enumerate(records):
        missing = {"id", "source", "target"} - set(record)
        if missing:
            raise PipelineError(f"GRPO record {index} is missing fields: {sorted(missing)}")
    ...
    if not prepared:
        raise PipelineError("GRPO split is empty.")
    return prepared
...
def load_grpo_records(data_dir: str | Path, dataset_key: str, split: str) -> list[dict[str, Any]]:
    ...
    if split not in {"train", "dev"}:
        raise PipelineError(f"Unsupported GRPO split '{split}'.")
