...
def evaluate_grpo_split(
    ...
    """Decode a P5 adapter greedily on the configured evaluation split."""
    if evaluation_file is None and split not in VALID_EVAL_SPLITS:
        raise PipelineError(f"Unsupported evaluation split: {split}")
    ...
    if ablation not in config["reward"]["ablations"]:
        raise PipelineError(f"Unknown P5 ablation: {ablation}")
